...
// Copied and modified from SampleScreenSpaceShadowmap from Shadows.hlsl. 
half VolumetricSampleScreenSpaceShadowmap(float4 shadowCoord)
{
    shadowCoord.xy /= max(0.00001, shadowCoord.w); // Prevent division by zero.

    // The stereo transform has to happen after the manual perspective divide
    shadowCoord.xy = UnityStereoTransformScreenSpaceTex(shadowCoord.xy);
    float2 positionSS = shadowCoord.xy * _ScreenParams.xy;
    
#if defined(UNITY_STEREO_INSTANCING_ENABLED) || defined(UNITY_STEREO_MULTIVIEW_ENABLED)
    half attenuation = LOAD_TEXTURE2D_ARRAY(_ScreenSpaceShadowmapTexture, positionSS.xy, unity_StereoEyeIndex).x;
#else
    half attenuation = half(LOAD_TEXTURE2D(_ScreenSpaceShadowmapTexture, positionSS.xy).x);
#endif
