...
{
    [CustomEditor(typeof(VolumetricCloudsHDRP))]
    public class VolumetricCloudsHDRPEditor : VolumeComponentEditor
    {
        ...
        {
            var o = new PropertyFetcher<VolumetricCloudsHDRP>(serializedObject);
        ...
        void LoadPresetValues(VolumetricCloudsHDRP.CloudPresets preset, bool microDetails)
        {
            switch (preset)
            {
                case VolumetricCloudsHDRP.CloudPresets.Sparse:
                    {
                    ...
                    break;
                case VolumetricCloudsHDRP.CloudPresets.Cloudy:
                    {
                        m_DensityMultiplier.value.floatValue = 0.4f;
                    ...
                    break;
                case VolumetricCloudsHDRP.CloudPresets.Overcast:
                    {
                    ...
                    break;
                case VolumetricCloudsHDRP.CloudPresets.Stormy:
                    {
        ...
        void SimpleControlMode(bool controlChanged)
        {
            ...
            using (new IndentLevelScope())
            {
                ...
                VolumetricCloudsHDRP.CloudPresets controlPreset = (VolumetricCloudsHDRP.CloudPresets)m_CloudPreset.value.enumValueIndex;
                VolumetricCloudsHDRP.CloudSimpleMode simpleQuality = (VolumetricCloudsHDRP.CloudSimpleMode)m_CloudSimpleMode.value.enumValueIndex;
                ...
                if (EditorGUI.EndChangeCheck() || controlChanged)
                {
                    ...
                    if (controlPreset != VolumetricCloudsHDRP.CloudPresets.Custom)
                    {
                        LoadPresetValues(controlPreset, simpleQuality == VolumetricCloudsHDRP.CloudSimpleMode.Quality);
                    }
                }
                ...
                using (new EditorGUI.DisabledScope(!(m_CloudPreset.overrideState.boolValue)))
                {
                    using (new IndentLevelScope())
                    {
                        ...
                        if (simpleQuality == VolumetricCloudsHDRP.CloudSimpleMode.Quality)
                        {
                            PropertyField(m_MicroErosionFactor);
                            PropertyField(m_MicroErosionScale);
                        }
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    if (controlPreset != VolumetricCloudsHDRP.CloudPresets.Custom)
                    {
                        m_CloudPreset.value.enumValueIndex = (int)VolumetricCloudsHDRP.CloudPresets.Custom;
                    }
                }
            }
        }
        ...
        bool CloudsShapeUI()
        {
            ...
            VolumetricCloudsHDRP.CloudControl previousControlMode = (VolumetricCloudsHDRP.CloudControl)m_CloudControl.value.enumValueIndex;
            PropertyField(m_CloudControl);
            VolumetricCloudsHDRP.CloudControl controlMode = (VolumetricCloudsHDRP.CloudControl)m_CloudControl.value.enumValueIndex;

            bool hasCloudMap = true;
            using (new IndentLevelScope())
            {
                if (controlMode == VolumetricCloudsHDRP.CloudControl.Advanced)
                    AdvancedControlMode();
                else if (controlMode == VolumetricCloudsHDRP.CloudControl.Manual)
                    ManualControlMode();
                else
                {
                    hasCloudMap = false;
        ...
        public override void OnInspectorGUI()
        {
            ...
            DrawHeader("Quality");
            {
                ...
                using (new IndentLevelScope())
                {
                    if ((VolumetricCloudsHDRP.CloudFadeInMode)m_FadeInMode.value.enumValueIndex == (VolumetricCloudsHDRP.CloudFadeInMode.Manual))
                    {
