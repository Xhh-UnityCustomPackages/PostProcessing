...
{
    curvature = 0.0;
    float3 offset = float3(_BlitTexture_TexelSize.zw, 0.0) * (CURVATURE_SCALE);
...
float4 SSC(Varyings input) : SV_Target
{
    float2 uv = input.texcoord;
    float3 sceneColor = SAMPLE_TEXTURE2D_LOD(_BlitTexture, sampler_LinearClamp, uv, 0);
    
    float cavity = 0.0, edges = 0.0, curvature = 0.0;
    
    #ifdef _TYPE_CURVATURE
        Curvature(uv, curvature);
    #endif
    
    #ifdef _TYPE_CAVITY
        Cavity(uv, cavity, edges);
    #endif
    
    float screenSpaceCavity = clamp((1.0 - cavity) * (1.0 + edges) * (1.0 + curvature), 0.0, 4.0) * 0.25;
    return screenSpaceCavity;
    return half4(sceneColor + screenSpaceCavity, 1.0);
}