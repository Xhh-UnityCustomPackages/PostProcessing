Shader "Hidden/PostProcessing/Bloom"
{
  ...
  uv + float2(0.0, texelSize * 1.38461538)));
    ...
    {
        float2 uv = UnityStereoTransformScreenSpaceTex(input.texcoord);
        half3 sceneColor = DecodeHDR(SAMPLE_TEXTURE2D_X(_BlitTexture, sampler_LinearClamp,uv));
        half3 bloomTex = DecodeHDR(SAMPLE_TEXTURE2D_X(_SourceTexLowMip, sampler_LinearClamp, uv));
        half3 bloomResult = bloomTex * Intensity * Tint;
        return half4(sceneColor + bloomResult, 1);
    }
    ENDHLSL
