...
/////////////////////////////////////LInear3DTrace/////////////////////////////////////
float4 LinearTraceRay3DSpace(int NumSteps, float2 BlueNoise, float3 rayPos, float3 rayDir)
{
    ...
    {
        endDepth = SAMPLE_TEXTURE2D_LOD(_CameraDepthTexture, sampler_CameraDepthTexture, rayPos.xy, 0.0);
...
void rayIterations(in bool traceBehind_Old, in bool traceBehind, inout half2 P, inout half stepDirection, inout half end, inout int stepCount, inout int maxSteps, inout bool intersecting,
...
half2 invSize, inout half layerThickness)
{
    ...
    for (; (P.x * stepDirection) <= end && stepCount < maxSteps && !stop; P += dP, Q.z += dQ.z, k += dk, stepCount += 1)
    {
        ...
        sceneZ = SampleSceneDepth(hitPixel * invSize);
    ...
    in out half stepCount)
{
    ...
    rayIterations(traceBehind_Old, traceBehind, P, stepDirection, end, originalStepCount, maxSteps, intersecting, sceneZ, dP, Q, dQ, k, dk, rayZMin, rayZMax, prevZMaxEstimate, permute, hitPixel, invSize, layerThickness);
...
inline half3 GetPosition(half4 _MainTex_TexelSize, half4 _ProjInfo, half2 ssP)
{
    half3 P;
    P.z = SampleSceneDepth(ssP.xy).r;
    P = ReconstructCSPosition(_MainTex_TexelSize, _ProjInfo, ssP, P.z);
    return P;
}
    ...
    */
    ...
    float4 Hierarchical_Z_Trace(int HiZ_Max_Level, int HiZ_Start_Level, int HiZ_Stop_Level, int NumSteps, float thickness, float2 RayCastSize, float3 rayStart, float3 rayDir, Texture2D SceneDepth, SamplerState SceneDepth_Sampler)
    {
        ...
        for (int i = 0; i < NumSteps; i++)
        {
            ...
            float sampleMinDepth = SAMPLE_TEXTURE2D_LOD(SceneDepth, SceneDepth_Sampler, newSamplePos.xy, level);
