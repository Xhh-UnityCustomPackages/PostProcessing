Shader "Hidden/DebugExposure"
{
    ...
    float3 FragMetering(Varyings input) : SV_Target
    {
        ...
        float3 color = SAMPLE_TEXTURE2D_X_LOD(_DebugFullScreenTexture, sampler_LinearClamp, uv, 0.0).xyz;
        ...
        if (all(input.positionCS.xy < topRight))
        {
            ...
            float3 pipColor = _ExposureDebugParams.x > 0 ? float3(1.0f, 1.0f, 1.0f) : SAMPLE_TEXTURE2D_X_LOD(_SourceTexture, sampler_LinearClamp, scaledUV, 0.0).xyz;
